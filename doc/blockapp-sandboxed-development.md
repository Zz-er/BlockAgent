...
- 你交付的还是**一个 `AppManifest`**（`packages/core/src/apps/agent_identity.ts` 是最小范本）。
...
内置 5 个 app 全是这一档。把 `blockapp-development.md` 的范式照搬即可。**只有当你的代码不可信（第三方、agent 产出、要装外部不可控逻辑）时，才需要下面的沙箱写法。**

---

## 2. 沙箱 app（sandboxed）怎么写：四条铁律

沙箱 app 跑在**子进程**里，渲染由主进程的可信通用 builder 代劳。所以你要换一套写法：

### 铁律 1 — 不写 build，只产 state（声明式投影）
...
> ⚠ **状态**：ceiling 真实强制 + install reject + 跨进程沙箱载体都是 **[DESIGNED]（UH-2 落地）**；现状 ceiling 仍是 report-only。本节描述的是 UH-2 后的目标行为，便于作者按统一模型先行编写。

所以：沙箱 app 声明能力要**最小化**，只要真正需要的；想要更高能力，就得让 operator 把它标成可信。

### 铁律 4 — state 有界、JSON、受配额
...
2. **切沙箱**：加 `trust:'sandboxed'`，把 builder 换成 `projection` 声明（铁律 1），把任何"想直接渲染"的逻辑改成"写 state + emit 门铃"（铁律 2）。
3. **收紧能力**：核对命令的 `capabilities`，删掉沙箱拿不到的（删 pinned/物理删/cred/裸联网），install 时看是否被 ceiling reject（ceiling reject = UH-2 行为）。
