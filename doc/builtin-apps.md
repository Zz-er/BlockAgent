...
| `base` | 开 | **基础层**：一个 app 同时**执行内置工具**（文件 / Shell / HTTP）+ **记录并呈现** agent 的每条命令（成败 + 结果）与每条外部输入，按时间线——补上 agent「我刚做了什么、成功了吗」的缺失观察 | 最近动作时间线（工具/命令 + 输入，详细程度可配） |
...
## 3. base —— 基础层：工具执行 + 动作/观察账本

`base` 是 agent 的**基础层**,一个 app 同时做两件事:**执行内置工具** + **记录并呈现** agent 的一切动作与观察。它默认开、是不可误卸载的观察底座。

**为什么有它**:LLM 按「调用 → 结果」配对训练。早先 block-agent 把效果散在各 app 块里,且 agent 看得到自己的**失败**却看不到自己的**成功**(成功命令的结果被丢弃)→ 容易重复同一操作。`base` 给它补上「我刚做了什么、成功了吗」的账本,并**接管**了原 `tools`(工具调用结果)和 `runtime:command_error`(失败显示)的角色——那两个块已移除,`tools` app 也并入了 `base`。

**① 内置工具(执行)**

- `base.read_file` / `base.grep` / `base.bash` / `base.http_request` —— 读文件、文本搜索、运行 Shell、发 HTTP。
- 危险/受限工具带权限门:`base.bash` 需 agent **经过审批**(`op:dangerous`)、`base.http_request` 受目标主机白名单(`net:http`);另有 `enabled` 子集独立门控(没开的工具直接拒)。由统一的鉴权关口按调用者身份执行。
- 工具返回完整结果(进 `CommandResult.data`),由下面的账本在 `command_detail=3` 呈现——所以**用工具时 `command_detail` 至少 3**(默认就是)。

**② 动作/观察账本(记录 + 呈现)**

- **一条交织的时间线**:外部输入(紧凑元信息:类型 + 发送人 + 时间 + 预览,正文不重复——对话正文在 `messages`)+ agent 命令与结果(命令名 + 参数 + 成败 + 结果)。
- **两层**:进上下文的是**有界的最近窗口**;**完整历史**(完整参数/结果/输入)落本地 jsonl,经 `base.show` 按需取——「保存完整」与「上下文不重复」两者兼顾。
- **详细程度三级、命令/输入分开配**:`command_detail`(默认 3=完整)、`input_detail`(默认 2=预览),各带字符上限。① 名称+成败 ② +内容(截断) ③ +完整内容。
- 接收外部输入的 app(如 `messages`)在自己的 ingest 里调 `ctx.report_input(...)` 把输入喂进时间线(详见 [blockapp-development.md](./blockapp-development.md))。

**底座性质**:默认开,且**不能被 `/app uninstall base` 卸载**(失败 + 工具结果只在这儿)——真要不用,只能在 `block-agent.config.json` 里 `apps.base.enabled:false`,并接受失去这些可见性。`base.set_config`(窗口/阈值/两个 detail/字符上限)**只有使用者能调**。
