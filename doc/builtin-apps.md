...
| `tools` | 开 | 给 agent 暴露文件 / Shell / HTTP 等工具（**只负责执行**；调用与结果由 `actions` 统一呈现） | （无独立块） |
| `actions` | 开 | agent 的**动作/观察账本**：它执行的每条命令（成败 + 结果）和收到的每条外部输入，按时间线呈现——补上 agent「我刚做了什么、成功了吗」的缺失观察 | 最近动作时间线（命令 + 输入，详细程度可配） |
...
每个工具是一条命令。tools **只负责执行**——调用的请求和结果**不再有自己的块**，而是和别的命令一样,由 `actions`(下一节)统一呈现在动作时间线里(避免同一个调用在两处重复出现)。

- **工具**：读文件、文本搜索、运行 Shell、发 HTTP 请求等。工具仍返回完整结果(进 `CommandResult.data`),`actions` 在 `command_detail=3` 下呈现它——所以**用工具时 `command_detail` 至少要 3**(默认就是 3),否则工具输出看不到。
- **约束**：危险或受限的工具带着相应的权限要求——比如运行 Shell 时 agent 需要**经过审批**，发 HTTP 时受目标主机白名单约束。这些约束由统一的鉴权关口按调用者身份执行。
- 注：早先 tools 有自己的 `tools:recent` 块和 `tool_history_count` 配置;现已移除,显示统一交给 `actions`。

---

## 3.5 actions —— 动作/观察账本

**为什么有它**:LLM 是按「调用 → 结果」配对训练的。早先 block-agent 把效果散在各 app 块里、且 agent 看得到自己的**失败**(旧的 `command_error` 块)却看不到自己的**成功**(成功命令的结果被丢弃)——于是 agent 容易重复执行同一操作。`actions` 给它补上这条缺失的观察:一份「我刚做了什么、成功了吗」的账本。它**接管**了原 `tools:recent`(工具调用结果)和 `runtime:command_error`(失败显示)两者的显示角色——这两个块已移除。

- **呈现**:一条**交织的时间线**——
  - **外部输入**(谁发来了什么):紧凑元信息(类型 + 发送人 + 时间 + 预览),正文不重复(对话正文在 `messages` 里)。
  - **agent 命令 + 结果**(它做了什么、成不成):完整呈现命令名、参数、成败、结果。
- **两层**:渲染进上下文的是**有界的最近窗口**;**完整历史**(含完整参数/结果/输入)落在本地 jsonl,经 `actions.show` 按需取——所以「保存完整」与「上下文不重复」两者兼顾。
- **详细程度三级,命令/输入分开配**:`command_detail`(默认 3=完整)、`input_detail`(默认 2=预览),各带字符上限。① 名称+成败 ② +内容(截断) ③ +完整内容。**用工具时 `command_detail` 至少 3**(否则工具结果看不到)。
- **它是观察层底座**:默认开,且**不能被 `/app uninstall actions` 卸载**(失败+工具结果只在这儿)——真要不用,只能在 `block-agent.config.json` 里 `apps.actions.enabled:false`,并接受失去这两类可见性。
- **配置**:`actions.set_config` 调窗口大小/阈值/两个 detail/字符上限,**只有使用者能调**。
- 接收外部输入的 app(如 `messages`)在自己的 ingest 里调 `ctx.report_input(...)` 把输入喂进时间线(详见 [blockapp-development.md](./blockapp-development.md))。
