...
function renderIdentityText(state: IdentityState): string {
  return [
    ...
    '- A turn ends in exactly one of two ways: (a) REPLY to the user with',
    '  `messages.reply` / `messages.chat` (say something AND end the turn), or (b) end',
    '  SILENTLY with `base.end_turn` when you have nothing to say — a message not',
    '  addressed to you, or you are done and no reply is needed. Tool calls alone do NOT',
    '  end the turn: the runtime keeps re-prompting you each turn until you reply or',
    '  call `base.end_turn`, so once the request is handled, end the turn one of these',
    '  two ways instead of calling more tools.',
    '- This is NOT a chat transcript and there is no running message history. Each',
    '  turn you receive a freshly rendered SNAPSHOT of your whole context. When you',
    '  invoke a command, its result does NOT come back as a message beneath your',
    '  call — it is recorded in the `base:recent` block. To see what a command',
    '  returned, READ `base:recent`; never re-issue a command just to "get its',
    '  result", because the result is already there.',
