...
已实现：核心闭环、内置模块、交互式终端、模块的发现与装卸（含热卸载）、模块间按契约协作（声明接口 + 每轮渲染前按契约取数）、外部语义记忆对接（真实 Letta / 百炼 DashScope 跑通）、可信／沙箱的统一宿主模型（跨进程沙箱载体已落地并接入启动、有端到端 fork 测试；发行版暂未附带沙箱模块）。测试全绿：core 471 · cli 88 · memory_letta 44。
