...
# 真实 LLM：key 只从环境变量读（绝不写进配置文件 / 不入库）
export ANTHROPIC_API_KEY=<你的 key>       # bash
# 或 PowerShell：$env:ANTHROPIC_API_KEY = "<你的 key>"
npm start

# 无 key 也想看它跑：离线 mock provider
npm start -- --dry-run
```

启动后是交互式终端：直接打字 = 给 agent 发消息；`/` 开头 = 斜杠命令（`/help` 看全部，`/apps` 看已装/可装 app）。

LLM 配置（provider / model / base_url）可走 `block-agent.config.json`（仓库根，gitignored）或命令行 flag，优先级 **flags > 配置文件 > env > 默认**。
