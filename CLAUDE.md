...
This `apps/` layout is the **unified-host model**: one BlockApp shape, with in-process vs cross-process as config (`trust`/`host` on the manifest, resolved by `app/host.ts` `resolveHost`). Both carriers are now wired (UH-2 landed): trusted apps run in-process; a `sandboxed` manifest is forked into a child-process `ChildProcessHost` (`launch.ts` injects the production factory — fail-closed, it never degrades a sandboxed app to in-process), and the capability ceiling is **enforced at install** (an out-of-ceiling cap on a `sandboxed`/`agent_authored` app is rejected, not report-only). End-to-end fork coverage lives in core (`sandbox_e2e.test.ts`, `child_process_host.test.ts`). What is **not** there yet: no sandboxed app ships (all built-ins are trusted), and hot-installing one is UH-3. Design: `ai_com/design/blockapp-unified-host-architecture.md` (gitignored, local-only).
...
Config precedence (`packages/cli/src/config.ts`): **CLI flags > `block-agent.config.json` > env > compiled defaults**. The **API key is the one exception** — read from env only (`ANTHROPIC_API_KEY` / `OPENAI_API_KEY`), never from a flag or the config file, never logged. A `.env` at the root is auto-loaded. Providers: `anthropic`, `openai-compat` (DeepSeek, Ollama, vLLM, 百炼 — all read `OPENAI_API_KEY` and need a `base_url`), `mock` (`--dry-run`).

**root_dir** (`packages/cli/src/bootstrap.ts`, design `ai_com/design/root-dir-architecture.md`): a per-process root holding `.env`, `block-agent.config.json`, and all app data (`<root>/.block-agent/apps/<id>/`). Both entry points (`main.tsx`, `server/bin.ts`) share one `bootstrap(argv, env)` prologue: resolve the root → `path.resolve` → fail-fast if an explicit root is missing (unless `--create-root`) → mkdir `.block-agent/apps` → take a single-root `agent.lock` (O_EXCL pid; a live holder is refused, a stale one preempted) → load `<root>/.env`. The root comes from **only two sources** — `--root-dir <path>` and `BLOCK_AGENT_ROOT_DIR` — and `BLOCK_AGENT_ROOT_DIR` **must be ambient env** (the `.env` lives inside the root, so it isn't loaded yet; putting the var in `.env` is a no-op — the bootstrap paradox, §1). Default root = cwd, byte-identical to legacy. Internally the root is threaded as `storage_dir` (`loadConfig(argv, env, {rootDir, rootExplicit})`), so existing data callsites (`appsBaseDir`, `/app purge`) are unchanged; the old `--storage-dir`/`BLOCK_AGENT_STORAGE_DIR` survive as deprecated aliases (an explicit root wins over them). **core stays dir-agnostic** — root_dir is a cli/server concept only, never injected into `packages/core`.
